...
class DroidLauncherUiActivity : Activity() {
    ...
    private fun button(value: String, filled: Boolean = false): Button = Button(this).apply {
        ...
        isAllCaps = false
        textSize = 13f
        setTextColor(if (filled) Color.WHITE else this@DroidLauncherUiActivity.text)
    ...
    private fun gamePage() {
        ...
        left.addView(button("+  Add Account"), LinearLayout.LayoutParams(-1, dp(48)))
    ...
    private fun libraryPage(page: String) {
        pageArea.addView(section("Download · $page", "Modern launcher library browser"))
        val items = when (page) {
        ...
        items.forEach { item ->
            ...
            info.addView(label(if (page == "Search by ID") "Release  ·  Android ready" else "Managed item  ·  Profile storage", 12f))
    ...
    private fun aboutPage() {
        ...
        setting("Build", "Integrated UI", "Navigation, cards, download lists and settings shell")
    ...
    private fun setting(name: String, value: String, description: String) {
        ...
        c.addView(label(description, 12f))
        val line = LinearLayout(this).apply { gravity = Gravity.CENTER_VERTICAL }
    ...
    private fun launchExistingActivity() {
        ...
        if (parts.size == 2) {
            try {
                ...
                // Existing activity remains optional; the launcher UI must not crash.
            }
        }
    }
    ...
    for pattern in patterns:
        for match in re.finditer(pattern, text):
            ...
            if "android.intent.action.MAIN" not in block or "android.intent.category.LAUNCHER" not in block:
                continue
            activity = re.search(r'android:name="([^"]+)"', block)
            if not activity:
                continue
...
def add_activity(text: str) -> str:
    ...
    return text.replace("</application>", activity + "    </application>", 1)
...
def main() -> int:
    ...
    src.write_text(ACTIVITY.replace("__EXISTING_LAUNCHER_COMPONENT__", existing), encoding="utf-8")
