...
This script is the final source boundary before Gradle. It removes known generator
corruption, restores class/method scope invariants, removes the fake bootstrap gate,
and runs the repository's 1000-check quality harness when available.
"""
...
def main() -> int:
    ...
    if 'showBootstrapGate()' in source[source.find('override fun onCreate'):source.find('override fun onCreate') + 600]:
    ...
    print('[step349] page boundary, startup, orphan contracts, server/Java members, EditText mappings and class scope verified')
    return 0
