...
def main() -> int:
    ...
    if MARKER in s:
        ...
        return 0
    s = s.replace('    private fun libraryPage(page: String) {\n        // STEP341_DOWNLOAD_MANAGER_REFERENCE_GUI', '    private fun libraryPage(page: String) {\n        // STEP341_DOWNLOAD_MANAGER_REFERENCE_GUI\n        if (page == "World") { worldsPage(); return }')
    s = s.replace('        pageArea.addView(filterRow)\n\n        if (page != "Game") {', '        pageArea.addView(filterRow)\n\n        if (page == "Game") {\n            val worldsCard = roundedCard(16)\n            val worldsRow = LinearLayout(this).apply { gravity = Gravity.CENTER_VERTICAL; setPadding(dp(12), dp(6), dp(12), dp(6)) }\n            worldsRow.addView(label("Worlds", 15f, true), LinearLayout.LayoutParams(0, dp(48), 1f))\n            val worldsButton = button("Open Worlds", true)\n            worldsButton.contentDescription = "Open Worlds manager"\n            worldsButton.setOnClickListener { worldsPage() }\n            worldsRow.addView(worldsButton, LinearLayout.LayoutParams(dp(140), dp(48)))\n            worldsCard.addView(worldsRow)\n            pageArea.addView(worldsCard)\n        }\n\n        if (page != "Game") {')
    s = s.replace('    s = s.replace(\'"Shader Pack" -> listOf("Complementary", "BSL", "Sildur\\\'s Vibrant", "MakeUp - Ultra Fast")\', \'"Shader Pack" -> listOf("Complementary", "BSL", "Sildur\\\'s Vibrant", "MakeUp - Ultra Fast")\\n            "World" -> listOf("Download World" to "Minecraft worlds", "Import World" to "Local .zip world", "Create New World" to "In Minecraft")\')', '') if False else s
