...
# Discover the app-private Minecraft root instead of assuming a package-specific path.
...
if [ -z "$ROOT_REL" ]; then
  echo 'No app-private directory containing versions/ was found.' >&2
  exit 1
fi
...
"$ADB" shell run-as "$PACKAGE" sh -c "test -s '$ROOT_REL/versions/1.21.1/1.21.1.jar' && test -s '$ROOT_REL/versions/1.21.1/1.21.1.json' && find '$ROOT_REL/libraries' -type f | grep -q . && test -s '$ROOT_REL/assets/indexes/$(basename "$(find "$FIXTURE/assets/indexes" -type f -name '*.json' -print -quit)")'" > "$OUT/staged-fixture-check.txt"

# Start the real production launcher entry point, then follow its visible Play/Start control.
...
if grep -Eiq 'MinecraftLaunchManager.*launch|NativeGameBridge.*launchJava|LaunchPreflight.*valid|Minecraft.*starting|Step [1-6]/6:' "$OUT/logcat.txt"; then
  echo 'Step 182 reached the production Minecraft launch path without fatal Android/JVM/native errors.' | tee "$OUT/result.txt"
else
  echo 'Step 182 staged real Minecraft 1.21.1 files but no concrete Minecraft JVM launch marker was observed.' | tee "$OUT/result.txt"
  exit 1
fi
