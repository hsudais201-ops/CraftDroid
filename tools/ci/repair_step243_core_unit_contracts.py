#!/usr/bin/env python3
"""Step 243/248: harden core unit contracts and keep Ely.by URL construction testable."""
...
def find_one(root: Path, name: str) -> Path:
    ...
    if len(hits) != 1:
        raise SystemExit(f"[step248] expected exactly one {name}, found {len(hits)}")
    return hits[0]
...
def patch_sources(root: Path) -> None:
    ...
    if new not in s:
        raise SystemExit("[step248] AccountProviderType unknown-id fallback is not Microsoft")
    ...
    if 'val isFixtureId = assignedUuid.startsWith("test-"' not in s:
        raise SystemExit("[step248] local test profile opaque fixture-id guard missing")
    ...
    if not changed:
        ...
        if not all(item in s for item in required):
            raise SystemExit("[step248] LaunchFailureClassifier summary anchors missing")
    ...
    if new_scopes not in s:
        raise SystemExit("[step248] documented Ely.by OAuth scopes are missing")

    # Kotlin/JVM cannot expose both a companion @JvmStatic method and an instance
    # method with the same erased signature. Keep one canonical pure implementation
    # in the companion for deterministic tests, and give the runtime wrapper a distinct
    # name so production callers remain explicit and the JVM has no signature clash.
    companion_anchor = '    companion object {\n'
    static_fun = '''        /** Pure URL formatter usable without constructing the provider. */
        @JvmStatic
...
'''
    # Remove every previously generated companion formatter block, then add exactly one.
    while static_fun in s:
        s = s.replace(static_fun, "", 1)
    if companion_anchor not in s:
        raise SystemExit("[step248] Ely.by companion object anchor missing")
    s = s.replace(companion_anchor, companion_anchor + static_fun, 1)

    # Rename the instance API generated by the Step 153 source. Calls are patched below.
    instance_signature = '''    fun buildAuthorizationUrl(
        clientId: String = DEFAULT_CLIENT_ID,
        redirectUri: String = DEFAULT_REDIRECT_URI
    ): String {'''
    renamed_signature = '''    fun buildAuthorizationUrlForRuntime(
        clientId: String = DEFAULT_CLIENT_ID,
        redirectUri: String = DEFAULT_REDIRECT_URI
    ): String = Companion.buildAuthorizationUrl(clientId, redirectUri)'''
    if instance_signature in s:
        start = s.index(instance_signature)
        brace = s.index('{', start)
        depth = 0
        end = None
        for i in range(brace, len(s)):
            if s[i] == '{':
                ...
                if depth == 0:
                    ...
                    break
        if end is None:
            raise SystemExit("[step248] could not locate runtime URL builder body")
        s = s[:start] + renamed_signature + s[end:]
    elif 'fun buildAuthorizationUrlForRuntime(' not in s:
        raise SystemExit("[step248] Ely.by runtime URL builder is missing")

    if 'fun buildAuthorizationUrl(clientId: String, redirectUri: String): String {' in s:
        # Only the companion implementation may have this exact signature.
        if s.count('fun buildAuthorizationUrl(clientId: String, redirectUri: String): String {') != 1:
            raise SystemExit("[step248] Ely.by static formatter implementation is duplicated")
    if 'fun buildAuthorizationUrlForRuntime(' not in s:
        raise SystemExit("[step248] Ely.by runtime wrapper rename missing")
    ely.write_text(s, encoding="utf-8")

    # Update the real generated account dialog to call the distinct runtime method.
    dialog = find_one(root / "app/src/main/java", "AddAccountDialog.kt")
    ds = dialog.read_text(encoding="utf-8")
    old_call = 'elyProvider.buildAuthorizationUrl()'
    new_call = 'elyProvider.buildAuthorizationUrlForRuntime()'
    if old_call in ds:
        ds = ds.replace(old_call, new_call, 1)
    if new_call not in ds:
        raise SystemExit("[step248] AddAccountDialog runtime Ely.by URL call was not repaired")
    dialog.write_text(ds, encoding="utf-8")

    print("[step248] unknown account provider now fails closed to Microsoft")
    print("[step248] local test fixture ids remain isolated from online authentication")
    print("[step248] crash-classifier summaries preserve analyzer-visible failure names")
    print("[step248] Ely.by OAuth scopes include account_email")
    print("[step248] Ely.by uses one static formatter plus a distinct runtime wrapper")
...
'''
    ...
    if 'val dummyClient = fileSystem.getVersionJarFile(detail.id)' not in s:
        raise SystemExit("[step248] launch-command test client artifact fixture missing")
    test.write_text(s, encoding="utf-8")
    print("[step248] LaunchCommandBuilder unit fixture supplies a valid-size client artifact")
