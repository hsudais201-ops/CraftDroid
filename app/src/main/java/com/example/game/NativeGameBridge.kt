...
object NativeGameBridge {
    ...
    fun shutdownGlfw() { if (loaded) runCatching { nativeGlfwShutdown() }.onFailure { LauncherLogger.warn("GLFW shutdown failed: " + it.message) } }
    ...
    fun sendMouse(x: Float, y: Float, dx: Float, dy: Float, button: Int, down: Boolean) {
        if (!loaded) return
        try { nativeMouse(x, y, dx, dy, button, down) } catch (t: Throwable) { LauncherLogger.warn("JNI mouse input failed: " + t.message) }
    }

    fun sendMouseScroll(horizontal: Float, vertical: Float) {
        if (!loaded) return
        try { nativeMouseScroll(horizontal, vertical) } catch (t: Throwable) { LauncherLogger.warn("JNI mouse-scroll input failed: " + t.message) }
    }

    fun sendKey(key: Int, down: Boolean, modifiers: Int = 0) {
        if (!loaded) return
        try { nativeKey(key, down, modifiers) } catch (t: Throwable) { LauncherLogger.warn("JNI keyboard input failed: " + t.message) }
    }
    ...
    fun sendGamepadButton(button: Int, down: Boolean) {
        if (!loaded) return
        try { nativeGamepadButton(button, down) } catch (t: Throwable) { LauncherLogger.warn("JNI gamepad button failed: " + t.message) }
    }
