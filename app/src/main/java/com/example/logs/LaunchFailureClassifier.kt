    ...
    fun classify(text: String): Result {
        ...
        return when {
            ...
            "unsatisfiedlinkerror" in t || "cannot open shared object file" in t || "dlopen failed" in t -> {
                ...
                Result(Kind.NATIVE_LIBRARY, "An Android native library could not be linked.", lib, "Verify ABI, LD_LIBRARY_PATH, JNI symbols, and the exact LWJGL/renderer native package.")
            }
            ...
            "unsupportedclassversionerror" in t || "more recent version of the java runtime" in t -> Result(Kind.JAVA_VERSION, "The selected Java runtime is incompatible with the Minecraft classes.", null, "Install/select the Java major required by the Minecraft version.")
            "outofmemoryerror" in t || "killed by android" in t -> Result(Kind.MEMORY, "The JVM/device ran out of memory.", null, "Lower the maximum heap and close background applications.")
